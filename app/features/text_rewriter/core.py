...
def executor(
    ...
    try:
        ...
        output = AITextRewriter(
            args=ai_text_rewriter_args, verbose=verbose
        ).rewrite_text(docs)

        logger.info("AI-TextRewriter rewrote a text successfully")

    except LoaderError as e:
        error_message = e
        logger.error(
            f"Error in AI Resistant Assignment Generator Pipeline -> {error_message}"
        )
